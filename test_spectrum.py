...
if dwf.FDwfDeviceOpen(-1, byref(hdwf)) != 1 or hdwf.value == consts.hdwfNone.value:
...
dwf.FDwfAnalogOutNodeEnableSet(hdwf, c_int(0), consts.AnalogOutNodeCarrier, c_int(1))
dwf.FDwfAnalogOutNodeFunctionSet(hdwf, c_int(0), consts.AnalogOutNodeCarrier, wavegen.function.sine)
dwf.FDwfAnalogOutNodeFrequencySet(hdwf, c_int(0), consts.AnalogOutNodeCarrier, c_double(500000))
dwf.FDwfAnalogOutNodeAmplitudeSet(hdwf, c_int(0), consts.AnalogOutNodeCarrier, c_double(0.705))
...
dwf.FDwfSpectrumWindow(byref(rgdWindow), c_int(nSamples), consts.DwfWindowFlatTop, vBeta, byref(vNEBW))
...
rgBins1 = np.fromiter(rgdBins1, dtype = np.float)
rgPhase1 = np.fromiter(rgdPhase1, dtype = np.float)

plt.title("FFT dBV-deg / MHz")
plt.xlim([0, hzTop/1e6])
plt.ylim([-180.0, 180.0])
#plt.xticks(np.arange(0, hzTop/1e6, hzTop/2e7))
